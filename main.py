...
def scrap(x):
    ...
    url = "https://www.offshore.org.gr/index.php?mx=Race_Schedule_2022&x=Program.xsl"
    xml = requests.get(url)
    soup = BeautifulSoup(xml.content, 'lxml')
    xml_tag = soup.findAll('district', attrs={'index':x})
    disctrict = str(xml_tag)
    disctricts = BeautifulSoup(disctrict, 'lxml')
    regattes = disctricts.findAll('regatta')

    f = open('cal.csv', 'w', newline='')
    writer = csv.writer(f)

    for i in range(len(regattes)):
        if(i == 0):
            if(x == 0):
                agwnes = "Σαρωνικός", ""
            elif(x == 3):
